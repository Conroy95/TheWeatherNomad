...
async function createWeatherCard(location){
    ...
    try{
        ...
        const data = await fetch(`https://wttr.in/${name}?format=%t;%f;%p;%w;%h;%S+%s;%C;%D`).then(r=>r.text());
        const [temp, feels, precip, wind, humidity, sun, condText, windDir] = data.split(";");

        document.getElementById(condition).innerText = condText;

        // Windrichting icoon URL (simple rotation based on cardinal points)
        const windIconMap = {
        ...
        <div class="icon-block">
            <img src="https://img.icons8.com/fluency/48/000000/wind.png" style="transform:rotate(${windDeg}deg)"/>
